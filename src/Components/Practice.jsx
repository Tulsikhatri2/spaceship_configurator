...
import { colorCalculation, powerCalculation, warpDriveCalculation,packageCalculation } from '../Redux/AmountCalculation/CalculationSlice';
  ...
  return (
    <div>
         ...
         <Grid item sx={{ height: "17vh" }} md={4}>
              <Box  className={`${ Info?.color ? "colorBox" : "" }` || `${Info.detail ? "packageBox":""}` ||
               `${Info || Info ? "driveBox":""}`} onClick={()=>{calculation(Info)}}
              >
                { Info?.color?
                (
